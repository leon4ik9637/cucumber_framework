...
//        Assert.assertFalse(heroAppPage.deleteButton.isEnabled());
    }


    @Then("user should see {string} text")
    public void user_should_see_text(String paragraphText) {
        switch (paragraphText){
            ...
            case "Right-click in the box below to see one called 'the-internet'. When you click it, it will trigger a JavaScript alert.":
                ...
                break;
            default:
                throw new NotFoundException("The paragraph is not defined properly in the feature file!!!");
        }
    }

    @Then("user should see a rectangle box to right click")
    public void user_should_see_a_rectangle_box_to_right_click() {
        Assert.assertTrue(heroAppPage.rectangleBox.isDisplayed());
    }

    @When("user right clicks on rectangle box")
    public void user_right_clicks_on_rectangle_box() {
        ActionsUtil.rightClick(heroAppPage.rectangleBox);
    }

    @Then("user should see a popup displaying message {string}")
    public void user_should_see_a_popup_displaying_message(String alertText) {
